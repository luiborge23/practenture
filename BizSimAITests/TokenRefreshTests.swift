...
final class TokenRefreshTests: XCTestCase {
    ...
    func test401TriggersTokenRefresh() async {
        ...
        let url = URL(string: "http://localhost:8005/api/health")!
    ...
    func testConcurrent401sCoalesce() async {
        ...
        async let r1: (Data, URLResponse) = try session.data(for: URLRequest(url: URL(string: "http://localhost:8005/api/health")!))
        async let r2: (Data, URLResponse) = try session.data(for: URLRequest(url: URL(string: "http://localhost:8005/api/sessions/TEST01")!))
