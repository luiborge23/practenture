...
import com.bizsimai.android.network.ApiFactory
...
class BackendAuthoritativeContractTest {
    ...
    private lateinit var repository: BizSimRepository

    @Before
    fun setUp() {
        server = MockWebServer().also { it.start() }
        repository = BizSimRepository(ApiFactory.create(server.url("/").toString()))
    }
    ...
    fun statusUsesBearerAuthentication() = runTest {
        val authenticatedRepository = BizSimRepository(
