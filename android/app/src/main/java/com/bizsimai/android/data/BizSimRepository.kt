...
class BizSimRepository(private val api: BizSimApi) {
    ...
    suspend fun login(username: String, password: String) = api.login(LoginRequest(username = username, password = password))
    suspend fun join(code: String, teamName: String, studentId: String) = api.join(code.uppercase(), JoinRequest(teamName, studentId))
