        ...
        try {
            ...
            if (account != null) {
                ...
                if (idToken != null) {
                    ...
                    val repo = BizSimRepository.create()
...
private fun LoginScreen(onLogin: (com.bizsimai.android.network.LoginResponse) -> Unit) {
    ...
    val repo = remember { BizSimRepository.create() }
    var username by remember { mutableStateOf("") }
    var password by remember { mutableStateOf("") }
    var message by remember { mutableStateOf("Sign in with your BizSimAI account") }
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(24.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        Text(com.bizsimai.android.BuildConfig.BIZSIMAI_BASE_URL, style = MaterialTheme.typography.bodySmall)
...
private fun SessionScreen(token: String, role: String, userId: String, prefs: android.content.SharedPreferences, onLogout: () -> Unit) {
    val scope = rememberCoroutineScope()
    val repo = remember(token) { BizSimRepository.create(token) }
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            Column { 
                Text("BizSimAI", style = MaterialTheme.typography.headlineMedium)
