...
// MARK: - practenture Simulation Engine
...
final class SimulationEngine: SimulationEngineProtocol {
    ...
    func processRound(
        ...
        for (team, decision) in teamContexts {
            ...
            let incentiveCost = decision.incentivePay * Double(workersNeeded) // per-worker, not per-unit
            ...
            let newEquity = team.equity + profit
            let equityForRatio = max(newEquity, 1.0)
            ...
            let roeDenominator = max(team.equity, config.initialEquity * 0.1)
            let roe = profit / roeDenominator
            let debtToEquity = equityForRatio > 0 ? newDebt / equityForRatio : 10
