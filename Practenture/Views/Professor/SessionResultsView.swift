...
final class CSVExportViewModel {
    ...
    func exportGrades() async {
        ...
        do {
            ...
            panel.nameFieldStringValue = "practenture_grades.csv"
            ...
            let fileURL = tempDir.appendingPathComponent("practenture_grades.csv")
    ...
    func exportLeaderboard() async {
        ...
        do {
            ...
            let panel = NSSavePanel()
            panel.allowedContentTypes = [.commaSeparatedText]
            panel.nameFieldStringValue = "practenture_leaderboard.csv"
            let result = await NSApplication.shared.begin { response in
                if response == .OK, let url = panel.url {
                    try? csv.write(to: url, atomically: true, encoding: .utf8)
                }
            ...
            #else
            let tempDir = FileManager.default.temporaryDirectory
            let fileURL = tempDir.appendingPathComponent("practenture_leaderboard.csv")
            try csv.write(to: fileURL, atomically: true, encoding: .utf8)
            await MainActor.run {
                isExporting = false
                exportSuccess = true
