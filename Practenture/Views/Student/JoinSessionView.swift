...
struct JoinSessionView: View {
    ...
    private func startQuickDemo() {
        ...
        NSLog("[Practenture] startQuickDemo: calling runQuickDemo now")
        appState.gameController?.runQuickDemo()
        NSLog("[Practenture] startQuickDemo: runQuickDemo returned")
    }
    ...
    private func onTeamJoined(teamId: UUID?) {
        ...
        NSLog("[Practenture] JoinSessionView: fetching results for session \\(sessionCode) to restore history")
        Task {
            do {
                ...
                if !backendResults.isEmpty {
                    await MainActor.run {
                        ...
                        if let activeSession = appState.activeSession {
                            NSLog("[Practenture] JoinSessionView: restoring \\(backendResults.count) rounds of history")
                            activeSession.restoreResultsFromBackend(backendResults)
                        }
                    }
                } else {
                    NSLog("[Practenture] JoinSessionView: no backend results yet (session may be new)")
                }
            } catch {
                NSLog("[Practenture] JoinSessionView: failed to restore history: \\(error)")
            }
        }
    }
}
