...
struct RoundResultsView: View {
    ...
    private var investorScorecardSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Text("Score: \(String(format: "%.0f", viewModel.investorScore))/100")
                    .font(.title3)
                    .fontWeight(.bold)
                    .foregroundStyle(viewModel.investorScore >= 70 ? PractentureTheme.success : viewModel.investorScore < 40 ? PractentureTheme.error : PractentureTheme.warning)
            }
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(
                    LinearGradient(
                        colors: [PractentureTheme.accentColor.opacity(0.06), PractentureTheme.accentColor.opacity(0.02)],
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(PractentureTheme.accentColor.opacity(0.08), lineWidth: 0.5)
        )
        .shadow(color: PractentureTheme.accentColor.opacity(0.04), radius: 6, x: 0, y: 3)
    }
    ...
    private func scorecardItem(_ label: String, value: String, score: Double, maxScore: Double) -> some View {
        VStack(spacing: 6) {
            Text(value)
                .font(.subheadline)
                .fontWeight(.bold)
                .foregroundStyle(score >= maxScore * 0.7 ? PractentureTheme.success : score < maxScore * 0.3 ? PractentureTheme.error : PractentureTheme.textPrimary)
        ...
        .background(
            RoundedRectangle(cornerRadius: 10)
                .fill(
                    LinearGradient(
                        colors: [PractentureTheme.secondary, Color.clear],
                        startPoint: .top,
                        endPoint: .bottom
                    )
                )
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 10) {
                channelCard(title: "Wholesale", revenue: viewModel.wholesaleRevenue,
                            units: viewModel.wholesaleUnitsSold, color: PractentureTheme.accentColor)
                channelCard(title: "Internet", revenue: viewModel.internetRevenue,
                            units: viewModel.internetUnitsSold, color: PractentureTheme.accentColor.opacity(0.7))
            }
            HStack(spacing: 10) {
                channelCard(title: "Amazon", revenue: viewModel.amazonRevenue,
                            units: viewModel.amazonUnitsSold, color: PractentureTheme.warning)
                channelCard(title: "Private Label", revenue: viewModel.privateLabelRevenue,
                            units: viewModel.privateLabelUnitsSold, color: PractentureTheme.success)
            }
        }
    }
    ...
    private var incomeStatementSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 0) {
                ...
                incomeDivider
                incomeRow("Gross Revenue", value: viewModel.revenue, isHeader: true, color: PractentureTheme.success)
                ...
                incomeRow("Net Income", value: netIncome, isHeader: true,
                          color: netIncome >= 0 ? PractentureTheme.success : PractentureTheme.error)
            }
        }
    }
    ...
    private func incomeRow(_ label: String, value: Double, isHeader: Bool, color: Color? = nil) -> some View {
        HStack {
            Text(label)
                ...
                .foregroundStyle(isHeader ? PractentureTheme.textPrimary : PractentureTheme.textSecondary)
            Spacer()
            Text(viewModel.formatted(abs(value)))
                .font(isHeader ? .subheadline : .caption)
                .fontWeight(isHeader ? .bold : .medium)
                .monospacedDigit()
                .foregroundStyle(color ?? (value < 0 ? PractentureTheme.error : PractentureTheme.textPrimary))
        }
    ...
    private var incomeDivider: some View {
        Rectangle()
            .fill(PractentureTheme.textSecondary.opacity(0.3))
            .frame(height: 1)
            .padding(.horizontal, 12)
            .padding(.vertical, 2)
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(columns: [
                ...
                resultCard(title: "Total Revenue", value: viewModel.formattedRevenue,
                           icon: "arrow.up.right.circle.fill", color: PractentureTheme.success,
                ...
                resultCard(title: "S/Q Rating", value: String(format: "%.1f", viewModel.sqRating) + "★",
                           icon: "star.fill", color: viewModel.sqRating >= 7 ? PractentureTheme.success : PractentureTheme.warning,
            ...
            LazyVGrid(columns: [
                ...
                resultCard(title: "Market Share", value: viewModel.formattedMarketShare,
                           icon: "chart.pie.fill", color: PractentureTheme.accentColor.opacity(0.7),
                           trend: viewModel.marketShare > 0.2 ? .up : .flat)
                resultCard(title: "Cash Balance", value: viewModel.formattedCashAfter,
                           icon: "banknote.fill", color: PractentureTheme.success.opacity(0.7),
                           trend: .flat)
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if viewModel.competitorSummary.isEmpty {
                ...
                ForEach(viewModel.competitorSummary) { comp in
                    ...
                    .padding(10)
                    .background(RoundedRectangle(cornerRadius: 8).fill(PractentureTheme.secondary))
                }
            }
        }
    }
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 10, style: .continuous)
                                .fill(explanation.impact == .positive
                                      ? PractentureTheme.success.opacity(0.04)
                                      : explanation.impact == .negative
                                      ? PractentureTheme.error.opacity(0.04)
                                      : PractentureTheme.textSecondary.opacity(0.04))
                        )
                    }
                }
            }
    ...
    private var coachingSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if viewModel.coachingTips.isEmpty {
                ...
                .background(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .fill(PractentureTheme.accentColor.opacity(0.04))
                )
            }
        }
    }
