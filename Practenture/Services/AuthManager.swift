...
final class AuthManager {
    ...
    func startTokenRefreshScheduler() {
        ...
        refreshSchedulerTask = Task { [weak self] in
            ...
            while !Task.isCancelled {
                ...
                _ = await self.proactiveTokenRefresh()
    ...
    @MainActor
    func loginWithApple(credential: AuthenticationServices.ASAuthorizationCredential, professorCode: String? = nil, providerNonce: String? = nil) async throws -> AuthLoginResponse {
        let idToken: String
        ...
        let response: AuthLoginResponse = try await network.post("/api/auth/login", body: AuthLoginRequest(provider: "apple", username: nil, password: nil, idToken: idToken, mfaCode: nil, professorCode: professorCode, providerNonce: providerNonce))
    ...
    @MainActor
    func loginWithGoogle(idToken: String, professorCode: String? = nil, providerNonce: String? = nil) async throws -> AuthLoginResponse {
        let response: AuthLoginResponse = try await network.post("/api/auth/login", body: AuthLoginRequest(provider: "google", username: nil, password: nil, idToken: idToken, mfaCode: nil, professorCode: professorCode, providerNonce: providerNonce))
        if response.professorCodeRequired == true {
            return response
        }
        persistTokens(access: response.accessToken, refresh: response.refreshToken)
    ...
    @MainActor
    func loginWithOAuth(provider: String, idToken: String, professorCode: String? = nil, providerNonce: String? = nil) async throws -> AuthLoginResponse {
        let response: AuthLoginResponse = try await network.post("/api/auth/login", body: AuthLoginRequest(provider: provider, username: nil, password: nil, idToken: idToken, mfaCode: nil, professorCode: professorCode, providerNonce: providerNonce))
        if response.professorCodeRequired == true {
            return response
        }
        persistTokens(access: response.accessToken, refresh: response.refreshToken)
