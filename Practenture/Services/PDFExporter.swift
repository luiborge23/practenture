...
private struct PDFRenderer {
    ...
    func generate() -> URL? {
        let fileURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("Practenture_Results_\(session.sessionCode).pdf")
        ...
        let data = renderer.pdfData { context in
            ...
            // Title
            let title = "Practenture — Simulation Results"
            ...
            // Footer
            y = ensureSpace(y: &y, needed: lineHeight, context: context)
            let footer = "Generated by Practenture • \(Date().formatted(date: .numeric, time: .standard))"
