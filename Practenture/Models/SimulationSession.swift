...
class SimulationSession: Identifiable {
    ...
    private static let logger = Logger(subsystem: "com.practenture", category: "SimulationSession")
    ...
    func restoreResultsFromBackend(_ backendResults: [Int: [RoundResultBackend]]) {
        NSLog("[Practenture] restoreResultsFromBackend: received \(backendResults.count) rounds")
        ...
        for (round, resultArray) in backendResults.sorted(by: { $0.key < $1.key }) {
            ...
            for backendResult in resultArray {
                ...
                guard let teamUUID = nameToUUID[backendResult.teamId] else {
                    NSLog("[Practenture] restoreResultsFromBackend: team '\(backendResult.teamId)' not found in local teams")
                    continue
                }
        ...
        NSLog("[Practenture] restoreResultsFromBackend: DONE — \(roundResults.count) teams with results, currentRound=\(currentRound)")
    }
    ...
    func takeSnapshot() -> SimulationSnapshot {
        NSLog("[Practenture] takeSnapshot: reading config...")
        let snapConfig = config
        NSLog("[Practenture] takeSnapshot: reading currentRound...")
        let snapRound = currentRound
        NSLog("[Practenture] takeSnapshot: reading teams...")
        let snapTeams = teams
        NSLog("[Practenture] takeSnapshot: reading decisions...")
        let snapDecisions = currentRoundDecisions
        NSLog("[Practenture] takeSnapshot: reading prevDecisions...")
        let snapPrevDecisions = previousRoundDecisions
        NSLog("[Practenture] takeSnapshot: reading roundResults...")
        let snapRoundResults = roundResults
        NSLog("[Practenture] takeSnapshot: building struct...")
        ...
        NSLog("[Practenture] takeSnapshot: DONE")
        return snapshot
    }
