BizSimAI — Master Strategy Blueprint
...
BizSimAI is a real-time, self-hostable business simulation platform for MBA/undergrad classrooms. Unlike Capsim (15-min rounds), Marketplace (5-min), Cesim (10-min), BizSimAI delivers true real-time feedback via WebSockets, enabling teachable moments as market dynamics unfold.
...
BizSimAI = iOS student app + FastAPI backend + professor web console + simulation engine + analytics + adaptive AI.
...
US-005 Production JWT Config P1 — BIZSIMAI_JWT_SECRET required error if missing, EXPIRY_HOURS default 24, CORS_ORIGINS default *, HOST default 0.0.0.0 PORT 8000, health reflects — COMPLETED passes true.
...
Health:
• GET /api/health no auth — status healthy service bizsimai-backend config host/port/cors/jwt_secret_configured/jwt_expiry.
...
+--------------------------------------------------------------------------------+
|                          BizSimAI Platform LIVE                                 |
+--------------------------------------------------------------------------------+
|                                                                               |
|  +-------------------+   HTTP/WS REST   +------------------------------+      |
|  | iOS App (SwiftUI) | <------------->  | FastAPI Backend              |      |
|  | 83 files 19.3k LoC|                  | Docker: bizsimai-backend     |      |
...
| IP 18.215.180.58 SG 80/443/22 | bizsimai-net bridge isol + DNS backend:8005 |
+--------------------------------------------------------------------------------+
...
• CORSMiddleware allow_origins *, allow_credentials *, allow_methods *, allow_headers * (configurable via BIZSIMAI_CORS_ORIGINS for prod).
...
• Config: Debug.xcconfig BIZSIMAI_BACKEND_URL=http://18.215.180.58 env Debug, Release.xcconfig, Staging.xcconfig, Info.plist BIZSIMAI_BACKEND_URL string + NSExceptionDomains fix, project.pbxproj Debug build settings.

Infrastructure:
• Dockerfile multi-stage python:3.12-slim builder pip install --prefix /install requirements.txt system deps libpq5 ca-certificates curl tini non-root user bizsimai expose 8005 healthcheck curl -f http://localhost:8005/api/health entrypoint tini uvicorn main:app --host 0.0.0.0 --port 8005.
• docker-compose.yml services backend build Dockerfile container bizsimai-backend restart unless-stopped env JWT_SECRET PROFESSOR_USERNAME PASSWORD JWT_EXPIRY APPLE_AUDIENCE GOOGLE_AUDIENCE healthcheck 30s 3 retries network bizsimai-net bridge; nginx image nginx:1.25-alpine container bizsimai-nginx restart unless-stopped ports 80:80 volumes ./nginx.conf:/etc/nginx/conf.d/default.conf:ro depends_on backend service_healthy network bizsimai-net.
...
• ec2-deploy.sh REGION us-east-1 INSTANCE_TYPE t3.micro KEY_NAME bizsimai STATE .ec2-state.json, provision 7 steps verify prereqs aws jq, create SSH key pair check local ~/.ssh/bizsimai never overwrite Python write not shell redirect security tools intercept, SG bizsimai-sg 80/443/22, launch ami-0c101f26f147fa7fd 20GB gp3 tag Name=bizsimai-backend, wait running+status OK, save state, configure SSH remove old known_hosts -R wait SSH up to 40*5s test echo ready, install Docker dnf docker git wget jq, systemctl start enable, usermod -aG docker ec2-user, install docker-compose binary curl GitHub v2.24.0 /usr/local/bin/docker-compose chmod +x AL2023 no plugin, verify docker --version.
• deploy 4 steps gen JWT secret token_hex 32 + professor password token_urlsafe 16 .env BIZSIMAI_JWT_SECRET USERNAME professor PASSWORD generated EXPIRY 24 NGINX_HTTP_PORT 80, rsync backend excluding venv __pycache__ .git .ec2-state.json data.db -e ssh -i ~/.ssh/bizsimai, docker-compose down --remove-orphans, up -d --build, health poll http://PUBLIC_IP/api/health 30*3s expect status healthy.
...
1. Debug.xcconfig BIZSIMAI_BACKEND_URL highest priority overrides everything — baked at build time need clean build Cmd+Shift+K→R.
2. NetworkService.swift #if DEBUG block return "http://18.215.180.58"
3. Info.plist BIZSIMAI_BACKEND_URL string http://18.215.180.58
4. project.pbxproj Debug build settings BIZSIMAI_BACKEND_URL = "http://18.215.180.58"
...
CORS currently * dev, restrict to specific origins prod via BIZSIMAI_CORS_ORIGINS env.

5.5 File Structure (current live):
BizSimAI-ios/BizSimAI/
├── BizSimAI/ (iOS Xcode project 83 Swift)
...
│   └── ...
├── PRD/ bizsimai_prd.md, prd.json (US-005 etc), sota_research.md, prompt_generation.md
...
| Feature | BizSimAI | Capstone | Marketplace | Cesim Global | Hubro | StratX | Forio |
...
BizSimAI Competitive Advantages (6 Pillars):
...
• Session creation: Capsim 30 min → BizSimAI 2 min = 28 min saved per session × 3 sessions per course × 2 courses per year = 2.8 hours saved/professor/year.
...
"For professors who want to make business strategy tangible, BizSimAI is an open-source real-time simulation platform that turns your classroom into a living market — where every decision gets immediate feedback, so you teach at the speed of business, not the speed of batch processing."
...
10.7 Marketing Plan:
• Website bizsimai.com (need buy domain) landing pages /professor /student /demo /pricing /open-source.
...
Phase B Launch (Sep-Dec 2026): Launch Product Hunt + Hacker News "Show HN: Open-source real-time BizSim for education", conference AACSB, 20 paying managed institutions $5k = $100k ARR.
...
• Domain bizsimai.com purchase Route53 EC2 alias + certbot Nginx HTTPS + wss.
...
BIZSIMAI_JWT_SECRET required JWT signing; BIZSIMAI_JWT_EXPIRY_HOURS default 24; BIZSIMAI_CORS_ORIGINS default *; BIZSIMAI_HOST default 0.0.0.0; BIZSIMAI_PORT default 8005; BIZSIMAI_PROFESSOR_USERNAME default professor; BIZSIMAI_PROFESSOR_PASSWORD default generated at deploy time was bizsimai2026; BIZSIMAI_APPLE_AUDIENCE optional Apple audience; BIZSIMAI_GOOGLE_AUDIENCE optional Google audience; DATABASE_URL optional sqlite/file/postgres; NGINX_HTTP_PORT default 80; plus MFA etc.

14.7 iOS 4-Location Backend URL Update Runbook (from MOP):
File1 Debug.xcconfig BIZSIMAI_BACKEND_URL = http://18.215.180.58 highest priority overrides everything baked build time need clean Cmd+Shift+K→Cmd+R.
File2 Services/NetworkService.swift #if DEBUG block return "http://18.215.180.58"
File3 Info.plist key BIZSIMAI_BACKEND_URL string http://18.215.180.58 + NSExceptionDomains dict key 18.215.180.58 dict NSExceptionAllowsInsecureHTTPLoads true plus NSAllowsArbitraryLoads true fallback.
File4 project.pbxproj Debug build settings BIZSIMAI_BACKEND_URL.
...
Apple Sign-In 500 InvalidAudienceError + KeyError x — RCA RSA vs EC keys Apple JWKS RSA RS256 n/e not EC ES256 x/y use RSAAlgorithm.from_jwk; PyJWT validates aud claim even without expected audience when present in token so verify_aud False when BIZSIMAI_APPLE_AUDIENCE empty GoogleLogin validation pattern test Google first simpler token isolates Apple issues.
...
Instance i-0486ccaada7f1af8b t3.micro AL2023 ami-0c101f26f147fa7fd 20GB gp3 us-east-1 SG sg-0051248e09678e39e bizsimai-sg ports 80/443/22 public IP 18.215.180.58 Docker Docker Compose v2.24.0 containers bizsimai-backend healthy bizsimai-nginx up bizsimai-net bridge. SSH ssh -i ~/.ssh/bizsimai ec2-user@18.215.180.58 docker ps logs docker-compose logs -f restart docker-compose restart rebuild docker-compose up -d --build.
...
END OF MASTER STRATEGY BLUEPRINT — This document is the single source of truth for BizSimAI strategic narrative, PRD, HLD, journeys, competitive, SWOT, GTM, and execution status as of July 10, 2026.
