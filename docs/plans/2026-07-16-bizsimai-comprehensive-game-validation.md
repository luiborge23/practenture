# BizSimAI Comprehensive Game Validation Implementation Plan
...
- Read: `backend/main.py`, `backend/routers/*.py`, `BizSimAI/Services/NetworkService.swift`
...
**Files:**
- Modify: `BizSimAITests/DecisionContractSerializationTests.swift`
- Create: `BizSimAITests/BackendResponseContractTests.swift`
...
**Verification:** run backend fixture tests and the `BizSimAITests` scheme; expected zero encode/decode mismatch.
...
**Files:**
- Modify/create tests under `BizSimAITests/`
- Modify/create UI tests under `BizSimAIUITests/`
...
2. Back up `/data/bizsim.db` and verify backup readability.
...
```bash
xcodebuild -project BizSimAI.xcodeproj \
  -scheme BizSimAI \
