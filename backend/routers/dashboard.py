...
async def get_dashboard_sessions(user=Depends(get_current_user)):
    ...
    for s in sessions:
        ...
        result.append(DashboardSessionResponse(
            code=s.code,
            name=s.config.name,
            courseCode=s.config.courseCode,
            semester=s.config.semester,
            state=state_str,
            ...
            maxHumanTeams=s.maxHumanTeams,
            marketType=s.config.marketType.value,
            aiDifficulty=s.config.aiDifficulty.value,
            scoringMetric=s.config.scoringMetric.value,
            startingCash=s.config.startingCash,
            randomSeed=s.config.randomSeed,
            fixedCostsPerRound=s.config.fixedCostsPerRound,
            baseCostPerUnit=s.config.baseCostPerUnit,
            baseMarketDemand=s.config.baseMarketDemand,
            sharesOutstanding=s.config.sharesOutstanding,
            initialEquity=s.config.initialEquity,
            baseInterestRate=s.config.baseInterestRate,
            plantCapacity=s.config.plantCapacity,
